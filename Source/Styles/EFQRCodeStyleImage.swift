...
public class EFStyleImageParamsData {
    ...
    public init(style: EFStyleImageParamsDataStyle, scale: CGFloat, colorDark: CGColor, colorLight: CGColor) {
        self.style = style
        self.scale = scale
        ...
        for x in 0..<nCount {
            for y in 0..<nCount {
                if typeTable[x][y] == QRPointType.alignCenter || typeTable[x][y] == QRPointType.alignOther || typeTable[x][y] == QRPointType.timing {
                    ...
                    let pointOpacity: CGFloat = isDark ? opacityDark : opacityLight
                    let pointSize: CGFloat = hit ? 1 : size
                    if hit {
                        pointList.append("<rect opacity=\"\(pointOpacity)\" width=\"\(pointSize)\" height=\"\(pointSize)\" key=\"\(id)\" fill=\"\(pointColor)\" x=\"\(x.cgFloat + (1 - pointSize) / 2.0)\" y=\"\(y.cgFloat + (1 - pointSize) / 2.0)\"/>")
                        id += 1
                    } else {
                        switch type {
                        case .rectangle:
                            ...
                            break
                        case .round:
                            pointList.append("<circle opacity=\"\(pointOpacity)\" r=\"\(pointSize / 2.0)\" key=\"\(id)\" fill=\"\(pointColor)\" cx=\"\(x.cgFloat + 0.5)\" cy=\"\(y.cgFloat + 0.5)\"/>")
                            id += 1
                            break
                        }
                    }
