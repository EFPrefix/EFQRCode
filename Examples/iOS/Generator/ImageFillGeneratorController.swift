//
//  ImageFillGeneratorController.swift
//  iOS Example
//
...
extension ImageFillGeneratorController {
    ...
    override func viewDidLoad() {
        ...
        title = NSLocalizedString("Create ImageFill", comment: "Title on generator")
    ...
    func chooseMaskColor() {
        let alert = UIAlertController(
            title: Localized.Title.maskColor,
            message: nil,
            preferredStyle: .actionSheet
        )
        alert.addAction(
...
#if os(iOS)
// MARK: - EFColorPicker
extension ImageFillGeneratorController: UIPopoverPresentationControllerDelegate, EFColorSelectionViewControllerDelegate {
...
#if os(iOS)
extension ImageFillGeneratorController: UIImagePickerControllerDelegate {
            ...
            if let tryGIF = images.first(where: { $0.value?.isGIF == true }) {
                if case .gif(let data) = tryGIF.value {
                    if let animatedImage = AnimatedImage(data: data, format: .gif) {
                        ...
                        self.image = .animated(images: frames, duration: duration)
                    }
                }
            }
